...
void processPhysique2(GameObject *current, GameObject *terrain,
                      float deltaTime) {
  ...
  bool hitGround = rayIntersectsMesh(tf.position, glm::vec3(0.0f, -1.0f, 0.0f),
                                     terrain->mesh, tHit, groundNormal);
...
void processPhysique(GameObject *current, GameObject *terrain,
  ...
  float t;
  glm::vec3 normal;
  bool hitMesh = sphereCast(tf.position, endPos, sphereRadius / 2,
                            terrain->mesh, t, normal);
  ...
  if (velLen > 1e-4f) {
    ...
    if (hitMesh) {
      ...
      if (glm::dot(velocity, normal) < 0.0f) {
        glm::vec3 reflected =
            glm::reflect(velocity, normal) * restitutionFactor;
...
void spheresCollision(std::vector<GameObject *> gameObjects,
                      float sphereRadius) {
  for (int a = 0; a < gameObjects.size(); ++a) {
    for (int b = a + 1; b < gameObjects.size(); ++b) {
      if (a == 0 || b == 0)
        continue; // ignorer le terrain
...
void resolveSphereCollision(RigidBody &a, RigidBody &b, float radiusA,
                            float radiusB) {
...
float closestDistanceSegmentTriangle(const glm::vec3 &segA,
                                     const glm::vec3 &segB,
                                     const glm::vec3 &triA,
                                     const glm::vec3 &triB,
                                     const glm::vec3 &triC,
...
bool sphereCast(const glm::vec3 &start, const glm::vec3 &end, float radius,
  ...
  for (size_t i = 0; i < mesh.indices.size(); i += 3) {
    ...
    float t;
    float dist =
        closestDistanceSegmentTriangle(start, end, v0, v1, v2, normal, t);
